...
import { DataTable, FilterBar, SavedView, type DataTableColumn } from "@/components/DataTable";
...
export default function SitesPage() {
  ...
  const columns: DataTableColumn<SiteRow>[] = [
    ...
    { key: "health", header: "Health", render: (s) => <Health value={s.health} /> },
    { key: "status", header: "Status", render: (s) => <SiteStatusBadge status={s.status} /> },
    { key: "revenue", header: "Revenue", render: (s) => <span className="font-semibold tabular-nums">{s.monthly.length ? formatMoney(s.revenuePerMonthEur) : "—"}</span> },
    { key: "sessions", header: "Sessions", render: (s) => <span className="tabular-nums">{s.monthly.length ? formatNumber(s.sessionsPerDay) : "—"}/d</span> },
    { key: "uptime", header: "Uptime", render: (s) => <span className="tabular-nums">{s.monthly.length ? formatPercent(s.uptimePct, 1) : "—"}</span> },
    { key: "position", header: "Position", render: (s) => s.position },
    {
      key: "action",
      header: "Next action",
      render: (s) => s.openIncidents > 0 ? <Link href={`/sites/${s.id}`} className="badge bg-amber-50 text-warning">Resolve {s.openIncidents}</Link> : <Link href={`/sites/${s.id}`} className="text-xs font-bold text-brand-600">Review</Link>,
    },
  ];
